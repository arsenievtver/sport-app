...
export function AthleteCoachesHomePanel({ refreshKey }: { refreshKey?: string } = {}) {
  ...
  const title = headingByCount(activeCoaches.length);

  if (loading) {
    return (
      <div className="athlete-home-section">
        <h2 className="athlete-home-section__title">Мой тренер</h2>
  ...
  return (
    <div className="athlete-home-section">
      <h2 className="athlete-home-section__title">{title}</h2>
      <section className="athlete-home-coaches glass glass--panel">
        {activeCoaches.length === 0 ? (
          <div className="athlete-empty-state">
            ...
            <p className="athlete-empty-state__text">
              Пока нет тренера. Добавь в настройках по коду приглашения.
            </p>
          </div>
        ) : (
